...
export default function Content() {
  ...
  if (debounceSearch.trim()) {
    queryParams.tokenArr = debounceSearch;
  }

  const isSearchOrFilter = Object.keys(queryParams).length > 0;

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["AllContent", queryParams],
    queryFn: async () => {
      if (isSearchOrFilter) {
        return await searchAndFilter(queryParams);
      } else {
  ...
  let contents: any = [];
  if (data?.data) {
    contents = data.data.wordsArr || data.data;
  }
  ...
  //     contentType?: string;
  //   }) => {
  //     return await searchAndFilter(data);
  //   },
  //   onSuccess: (data) => {
  //     console.log(data);
  //     contents = data.data.wordsArr;
  //   },
  //   onError: (error) => {
  //     toast.error(error.message || "Failed to search content.");
  //   },
  // });
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setMultiDeleteContentIds(
        contents.map((content: ContentItem) => content._id)
      );
    } else {
      setMultiDeleteContentIds([]);
    }
  ...
  const areAllVisibleContentsSelected =
    ...
    contents.every((content: ContentItem) =>
      multiDeleteContentIds.includes(content._id)
    );
    ...
    <div className="w-full">
      <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-6 px-4 py-3 bg-white rounded-lg shadow-sm border border-gray-100">
        <div className="relative flex-grow w-full lg:max-w-xs">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400 z-0" />
          <Input
            type="text"
            placeholder="Search content by name or keyword..."
            className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all w-full z-0"
            value={searchTerm}
                    ...
                    <td className="px-4 py-4 text-sm text-gray-600">
                      <div className="flex flex-wrap gap-1">
                        {content.tags?.slice(0, 2).map((tag) => (
                          ...
                          >
                            <Tags className="w-3 h-3" /> {tag.value}
                          </Badge>
                        ))}
